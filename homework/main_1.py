...
import pictures_1
import prices_1
from discount_1 import DISCOUNT_TRIGGER_PEOPLE, DISCOUNT_PERCENTAGE

put_markdown("# 🏔️ Організатор подорожі в Карпати")
put_image(pictures_1.PICTURE_CARPATHIANS)
put_markdown("---")
...
if students == 0:
    ...
    if transport_type == '🚌 Автобус':
        put_image(pictures_1.PICTURE_BUS, width="300")
        buses_count = total_people // prices_1.BUS_CAPACITY
        if total_people % prices_1.BUS_CAPACITY > 0:
            buses_count += 1
        cost_transport = buses_count * prices_1.PRICE_BUS
    else:
        put_image(pictures_1.PICTURE_TRAIN, width="300")
        cost_transport = total_people * prices_1.PRICE_TRAIN

    cost_hotel = 0
    if days > 0:
        cost_hotel = total_people * prices_1.PRICE_HOTEL_NIGHT * days
